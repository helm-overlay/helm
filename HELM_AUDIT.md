...
- IN PROGRESS: Move the entire live-state lifecycle for both Claude and Pi into hooks/extensions and remove the separate live JSON registries (`~/.claude/sessions/<pid>.json`, `~/.pi/sessions/<pid>.json`) if possible. **Done so far:** the `helm` Claude plugin now writes run/attention state on every lifecycle event (running on start/prompt, needs_input on AskUserQuestion, verdict on Stop, cleared on end), and `SessionStore.resolveLiveRow` treats that state as authoritative for any live row — status is now hook-owned. **Remaining:** liveness itself (is the session alive, + its pid for kill) still comes from the registries; dropping them needs the pid carried in hook state (or another liveness signal) plus staleness handling, so the reaper no longer depends on the registry.
- DONE (Claude): `helm init claude` installs/updates the hooks plugin automatically (`ClaudeHooksPlugin` / `ClaudePluginInstaller`, CLI target `HelmCLI`). TODO: `helm init pi` to install the Pi extension the same way.
