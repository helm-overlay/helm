...
- **PR expiry:** `PRSource` drops a promoted PR from the feed once its `updatedAt` is older than `staleAfter` (7d) — untouched PRs stop nagging (still reachable via search). In the final `expiringWithin` (1d) before that cutoff, `source.expiringSoon(_:)` flags the row and `AttentionRow` draws a thin amber leading bar. `now`/`staleAfter`/`expiringWithin` are injected (defaulted) for testability. `expiringSoon` is a defaulted-`false` `AttentionSource` member, parallel to `promotes`.
